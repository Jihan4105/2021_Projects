...
function Lec_Lookup() {
    ...
    $.ajax({
        ...
        success: function (result) {
            var html = "";
            for (classNo in result.items) {
                html += '<tr>';
                html += '<td class="row-id">' + '<input type="checkbox" data-no="' + result.items[classNo].classNo + '" name="classNo">' + '</td>';
                html += '<td>' + result.items[classNo].classNo + '</td>';
                html += '<td>' + result.items[classNo].className + '</td>';
                html += '<td>' + result.items[classNo].time + '</td>';
                html += '<td>' + result.items[classNo].proFessor + '</td>';
                html += '</tr>';
            }
            $("#lec_list_id").empty();
...
function Lec_Register(classNo, sub, time, per) {
    var deferred = $.Deferred();
    $.ajax({
...
function Lec_Fix_Page_Button() {
    ...
    for (var i = 0, k = 0; i < nom.length; i++) {
        if (nom[i].checked == true) {
            $("#tmp_storage_id").data("classNo", nom[i].dataset.no);
