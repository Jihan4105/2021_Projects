...
function Lec_Class_Select() {
    var classNo = $("#sel1_id option:selected").data("no");
    Lec_Schedule_Lookup(classNo);
}

function Lec_Schedule_Lookup(classNo) {
    var deferred = $.Deferred();
    $.ajax({
...
function Lec_Sch_Register() {
    ...
    Sch_Register(classNo, date, time)
        .done(function () {
            alert("추가되었습니다!")
            Lec_Schedule_Lookup(classNo);
...
function Sch_Register(classNo, date, time) {
    var deferred = $.Deferred();
    $.ajax({
...
function Lec_Sch_Fix() {
    var raw_date = $("#schedule_datepicker_id").dxDateBox("instance").option('value');
    var id = $("input:checkbox[name='schedulechk']:checked").data("id");
    var month = raw_date.getMonth() + 1;
    var day = raw_date.getDate();
    if (month < 10) { month = '0' + month; }
    if (day < 10) { day = '0' + day; }
    ...
    Sch_Fix(global_selected_chk_no, date, time, id)
        .done(function () {
            alert("수정되었습니다!");
            Lec_Schedule_Lookup(classNo);
        })
...
function Sch_Fix(global_selected_chk_no, date, time, id) {
    var deferred = $.Deferred();
    $.ajax({
